...
- **AI Integration**: Leverages Google Vertex AI (gemini-2.5-flash) for document analysis, ensuring server-side processing for security and structured JSON output based on dynamic provider schemas. Supports dual OCR engine mode for admin users: "Vertex AI" (Gemini processes images directly) and "Vision OCR + Gemini" (Google Cloud Vision API extracts text first, then Gemini structures it). Non-admin users always use the default Vertex AI engine.
...
- **Audit Rules System**: Includes ~201 active medical report rules, covering general validations and provider-specific rules for GNP (32), MetLife (45), NYLife (45), AXA (50), and General (29). Rules use comma-separated providerTarget values (e.g., 'GNP,METLIFE', 'AXA,AXA_2018') to target compatible providers. NYLIFE uses unique field paths (e.g., `firma_pagina_2.fecha.formatted` for 6-month validity, `padecimiento_actual.diagnosticos` instead of `diagnostico.diagnostico_definitivo`, `equipo_quirurgico_nylife.*` for surgical team, `antecedentes_patologicos.*` / `antecedentes_no_patologicos.*` for medical history). NYLIFE has NO medication table and NO "referido" field. RFC validation for NYLIFE is IMPORTANTE level, only when `medico_tratante.nombres` exists. AXA supports two format versions: AXA 2025 (default, id='AXA', 6 pages, reference AI-461, table-based antecedentes patológicos, plan terapéutico, solicitud de material sections) and AXA 2018 (legacy, id='AXA_2018', 5 pages, reference AI-346, checkbox-based antecedentes). Common rules target 'AXA,AXA_2018'; 2018-specific rules (antecedentes checkboxes, datos personales, transferencia datos) target 'AXA_2018' only. Provider detection distinguishes versions via document markers (AI-461/piso 3 vs AI-346/Piso 6). AXA has its own field schema (e.g., `medico_principal.nombre`). The system supports 33 operators including array validators (ARRAY_EMPTY, ARRAY_LENGTH_GREATER_THAN, ARRAY_CONTAINS_NONE, ARRAY_MUTUALLY_EXCLUSIVE) and DATE_OLDER_THAN_MONTHS. All date parsing uses DD/MM/YYYY (Mexican format).
...
- **Free Trial System**: New users automatically receive 10 free medical report evaluations or 1 month of access (whichever expires first). Trial users have full platform access during the trial period. When the trial expires (reports exhausted or time elapsed), users are prompted to subscribe via the subscription modal. Trial fields (`isTrial`, `freeReportsUsed`, `freeReportsLimit`, `trialExpiresAt`, `trialConvertedAt`) are stored on the User model. An admin "Conversión de Pruebas" dashboard (`/api/trial/dashboard`) tracks trial metrics: total users, active/expired/converted counts, conversion rate, and average reports used.
