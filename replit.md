...
- **Audit Rules System**: Includes ~201 active medical report rules, covering general validations and provider-specific rules for GNP (32), MetLife (45), NYLife (45), AXA (50), and General (29). Rules use comma-separated providerTarget values (e.g., 'GNP,METLIFE') to target compatible providers. NYLIFE uses unique field paths (e.g., `firma_pagina_2.fecha.formatted` for 6-month validity, `padecimiento_actual.diagnosticos` instead of `diagnostico.diagnostico_definitivo`, `equipo_quirurgico_nylife.*` for surgical team, `antecedentes_patologicos.*` / `antecedentes_no_patologicos.*` for medical history). NYLIFE has NO medication table and NO "referido" field. RFC validation for NYLIFE is IMPORTANTE level, only when `medico_tratante.nombres` exists. AXA has its own field schema (e.g., `medico_principal.nombre`). The system supports 33 operators including array validators (ARRAY_EMPTY, ARRAY_LENGTH_GREATER_THAN, ARRAY_CONTAINS_NONE, ARRAY_MUTUALLY_EXCLUSIVE) and DATE_OLDER_THAN_MONTHS. All date parsing uses DD/MM/YYYY (Mexican format).
